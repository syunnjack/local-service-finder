...
export default async function CityComparisonPage({ params }: { params: Promise<{ slug: string; city: string }> }) { const { slug, city: citySlug } = await params; const vertical = getVertical(slug); const city = getCity(citySlug); if (!vertical || !city) notFound(); return <VerticalPage vertical={vertical} initialCity={city.name} regionFact={getRegionFact(vertical.slug, city.slug)} />; }
