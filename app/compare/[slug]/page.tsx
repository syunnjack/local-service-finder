...
import { getRegionFact, getVerifiedCities } from "../../lib/region-facts";
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) { const { slug } = await params; const vertical = getVertical(slug); if (!vertical) notFound(); const citySlug = getVerifiedCities(vertical.slug)[0]; const city = citySlug ? getCity(citySlug) : undefined; const regionFact = citySlug ? getRegionFact(vertical.slug, citySlug) : undefined; return <VerticalPage vertical={vertical} initialCity={city?.name} regionFact={regionFact} />; }
