...
export default function VerticalPage({ vertical }: { vertical: Vertical }) {
  ...
  const [reviewService, setReviewService] = useState(vertical.services[0]);
  ...
  async function submitReview(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const response = await fetch("/api/reviews", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ vertical: vertical.slug, providerId: selected?.id || "", city, nickname, rating, body, serviceType: reviewService, usageMonth, wouldUseAgain }) });
    const data = (await response.json()) as { error?: string; message?: string };
    setReviewStatus(response.ok ? data.message || "口コミを受け付けました。確認後に掲載します。" : data.error || "送信に失敗しました。");
    if (response.ok) { setNickname(""); setBody(""); setUsageMonth(""); setWouldUseAgain(false); }
  }
